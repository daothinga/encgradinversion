...
class LeNet0(nn.Module):   # Number of input channels: 1
    def __init__(self, num_classes=100, n_h=768):   #n_h: Number of hidden units
        super(LeNet0, self).__init__()
        act = nn.Sigmoid
        self.conv1 = nn.Sequential(
...
class LeNet1(nn.Module):   # Number of input channels: 4
    def __init__(self, num_classes=100, n_h=768):   #n_h: Number of hidden units
...
value = 0.1
def weights_init(m, fc_bias_value=1.0):
    if hasattr(m, "weight"):
    ...
    ### Nga update 20260109
    # Special case: initialize the last fc layer bias with a constant 
    if hasattr(m, "fc") and hasattr(m.fc, "bias") and m.fc.bias is not None: 
        m.fc.bias.data.fill_(fc_bias_value)
    if hasattr(m, "fc") and hasattr(m.fc, "weight") and m.fc.weight is not None: 
        m.fc.weight.data.fill_(1.0)
...
class EfficientNet(nn.Module):
    ...
    def create_features(self, width_factor, depth_factor, last_channels):
        ...
        features_ = [CNNBlock(3, channels, 3, stride=2, padding=1)]
        ...
        return nn.Sequential(*features_)
...
def EfficinetNet0(version, num_classses):
    return EfficientNet(version=version, num_classes=num_classses).to(device)



class MobileNetV1(nn.Module):
    def __init__(self, ch_in, n_classes, n_h):
        super(MobileNetV1, self).__init__()

        self.n_h = n_h          # The number of hidden units in the last layer

        def conv_bn(inp, oup, stride, act=nn.Sigmoid()):
        ...
        def conv_dw(inp, oup, stride, act=nn.Sigmoid()):
            return nn.Sequential(
                # dw
        ...
        self.model = nn.Sequential(
            ...
            ##### Version 2
            # conv_bn(ch_in, 16, 2, nn.ReLU()),
            # conv_dw(16, 32, 2),
            # conv_dw(32, 64, 2, nn.ReLU()),
            # conv_dw(64, 64, 1),
            # conv_dw(64, 64, 1),

            # conv_bn(ch_in, 16, 2),
            # conv_dw(16, 32, 2, nn.ReLU()),
            # conv_dw(32, 64, 2, nn.ReLU()),
            # conv_dw(64, 64, 1),
            # conv_dw(64, 64, 1),
...
def MobileNet(ch_in=3, n_classes=100, n_h = 32*32):
    return MobileNetV1(ch_in, n_classes, n_h)
